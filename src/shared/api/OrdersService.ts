...
export class OrderService extends ServerService {
  async getOrders(currentPage: number, itemsPerPage: number, status: number | null): Promise<Order[]> {
  ...
  async getOrdersByTotal(currentPage: number, itemsPerPage: number, status: number | null, sortOrder: string) : Promise<Order[]> {
    const start = (currentPage - 1) * itemsPerPage;
  ...
  async getAllOrdersByTotal(status: number | null, sortOrder: string) : Promise<Order[]> {
    let endpoint = '/orders';
